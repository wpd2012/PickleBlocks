...
- **TypeScript 7**: typechecking and production builds use the native TypeScript 7 compiler. The main editor keeps the TypeScript 6 compatibility API installed alongside it for tools such as `typescript-eslint` until their TypeScript 7 API support lands.

## Current Checkpoint

PickleBlocks v0.6.7 is the active development milestone. It builds upon v0.6.5, introducing the **Icon-Only Segmented Scene View Control** (`[ ◧ | 🖼️ | ☰ ]` for Split View, Scene Canvas Only, and Actor Panel Only with compact 24px height, 1:1 square ratio buttons, and a subtle neutral monochrome design), the **Scene Toolbar Colliders Preference & Right-Click Context Menu** (`Show Colliders toggle in Scene toolbar` under Settings ➔ Scene, plus an interactive right-click context menu `Hide` option on the Colliders toggle across Editor-Scene and SceneEditor), persistent **Reorderable Main Panes & Workspace Header Docking**, the **Configurable Accent & Highlight Color System**, **TypeScript 7 Compiler Migration** (`@typescript/native` v7.0.2), scoped **Local Variables for Events & Functions**, dynamic **Typed Function Argument Sockets**, custom inline **Value Picker popover dropdowns**, a fully-featured **Hierarchical Entity Prefab System**, and an interactive **Blockly Workspace Block Search** toolbar.

### Advanced Custom JavaScript

The Blockly **Advanced** category includes a statement-only **Run JavaScript** action for trusted synchronous code. Its CodeMirror editor provides syntax highlighting, line numbers, search, bracket assistance, inline diagnostics, API completions, and starter snippets in both Blockly and the Inspector. Source is saved in project JSON, validated by the Blockly compiler, cached by block ID at runtime, and executed through one shared API implementation in Preview, Play Mode, and standalone exports. Imported projects containing custom code display a trust confirmation before loading.

Custom scripts receive an `api` object with:

- `variable(name)`, `setVariable(name, value)`, `local(idOrName)`, `setLocal(idOrName, value)`, and `parameter(idOrName)`.
- `actor(target)` and `actors(target)` read-only snapshots.
- Engine-safe commands including `setPosition`, `moveBy`, `setVelocity`, `applyForce`, `applyImpulse`, `setColor`, `setText`, `show`, `hide`, `destroy`, and `playAnimation`.
- `phase`, `deltaTime`, read-only `scene` metadata, and `log(...)`.

Custom JavaScript is an advanced escape hatch, not a security sandbox. It can access browser globals, must not use asynchronous control flow, and an infinite loop can freeze the game. Only run projects from sources you trust.

It also carries forward the full v0.6.1 features: a massive expansion of the Actor Behaviors System with 14 new specialized behaviors (such as platformer/top-down movement controllers, homing, orbit, bob, camera follow, lifetime destroy, and scene edges), support for physically throwing dragged entities, and dynamic sprite rotations for rolling/spinning physics bodies.

Subsequent updates to v0.5.2 include a comprehensive engine test suite expansion (191 tests across 14 suites covering geometry bounds, collision pair detection, collision provider queries, scene clamping, entity scaling, and event interpreter runtime), a centralized `COLORS` constant replacing magic color literals in the canvas renderer, O(1) indexed sprite and asset lookups replacing per-frame linear scans in the render loop, pre-allocated collision pair arrays in the provider, named frame-timing constants with documentation, NaN-safe geometry guards, audio element lifecycle cleanup on runtime stop to prevent memory leaks across project switches, a barrel re-export module for the engine package, documented empty catch blocks across the editor application, and a GitHub Actions CI workflow enforcing typecheck, lint, and test on every push and pull request.
...
- Decorative Props System: completely removed the legacy standalone `props` list from project specifications and schemas, fully unifying decorative props with the primary `entities` array. Pre-processing rules continue to seamlessly migrate legacy data to the unified `kind: 'prop'` format.
- Center-Pivot Coordinate System: shifted all coordinate parameters `(x, y)` to center-pivot anchors (rather than top-left corners) across editors, compilers, and rendering modules. Extracted a shared `Geometry.ts` library for calculating bounding bounds, circle colliders, containment checks, and distances.
- Custom Actor Behaviors: added a modular Actor Behavior framework (`behaviours` configurations) inside the entity details panel. Allows attaching, ordering, toggling, and adjusting speed/parameters for reusable behaviors: Drag & Drop, Directional Control, Spin, Auto Move, Patrol, and Follow Actor.
- Extended Physics Constraints: supports static, dynamic, and kinematic body types, custom body density, linear damping, horizontal and vertical axis locks, one-way platform logic, and layer-based collision filtering (assigning entities to default/player/world/enemy/item layers and specifying exact category collision masks).
- Visual Entity Thumbnails: renders active sprite frame thumbnails, raw image thumbnails, or custom SVG kind icons inside the entity details inspector header.
...
- Canvas 2D runtime split into dedicated engine modules with a barrel re-export entry point (`src/engine/index.ts`)
...
- **Geometry library** (`src/engine/Geometry.ts`) providing `getEntityBounds`, `getBoxColliderBounds`, `getCircleCollider`, `containsPoint`, and `getEntityCenterDistance` with NaN-safe division guards
- **Modular Collision Provider** (`src/engine/CollisionProvider.ts`) exposing a `RuntimeCollisionProvider` interface and `ManualCollisionProvider` implementation with pre-allocated pair arrays, allowing future spatial-index optimizations
- Renderer module with named `COLORS` constant, O(1) indexed sprite and asset lookups, rotated entities, interactive UI button backgrounds/borders, and transparent non-interactive text HUD overlays
- Responsive UI layout resolver with viewport/parent anchors, edge stretching, margins and offsets, nested panel/container flow layouts, padding, gaps, alignment, and deterministic child ordering
- Multiline UI text with explicit newlines, word wrapping, line-height and vertical alignment controls, optional clipping, and matching Preview/Scene Editor/standalone behavior
- Responsive UI Playground sample demonstrating an anchored HUD, centered menu container, wrapped copy, and a layout-aware interactive button
- Imported WOFF2, WOFF, TTF, and OTF font assets with per-element family, weight, style, fallback, Preview preloading, and standalone embedding
- Stateful UI buttons with configurable hover, pressed, focused, and disabled fills/text, visible focus rings, Inspector state previews, and disabled hit-test protection
- Automatic spatial keyboard/gamepad focus navigation with focus order, default focus, wrap controls, Enter/Space/controller activation, UI focus/pressed conditions, and focus/enable actions
- Accessible UI Controls sample demonstrating keyboard and controller menu navigation
- Reusable UI style presets with live contrast auditing, 9-slice backgrounds, explicit directional focus links, modal focus scopes, delayed tooltips, scrollable clipped containers, and UI focus/press sound cues
- Editable font asset families, weights, styles, names, and safe deletion with UI fallback preservation
- Named frame-timing constants (`MAX_DELTA_SECONDS`) documenting tab-switch/pause protection bounds
- Scene clamping using center-pivot coordinates with documented guard rails
- Polyphonic audio mixer with Music, Effects, and UI buses; master/bus volume, mute and voice limits; per-asset pitch, pan, random variation, loop points, fades and priority-based voice stealing
- Music crossfades plus advanced audio event/Blockly actions and audio-playing/bus-muted conditions, with Preview and standalone export parity
...
- **Comprehensive test suite**: 191 tests across 14 suites covering geometry, collision pairs, collision provider, scene clamping, entity scaling, renderer, input, groups, compiler, schemas, standalone exporter, function analysis, and full game runtime integration
- **CI workflow**: GitHub Actions enforcing TypeScript typecheck, ESLint, and Vitest on every push and pull request
...
### v0.6.4 - Configurable Accent & Highlight Color System, Unified Engine Button Design System, TypeScript 7 Migration, and Scoped Local Variables

- **TypeScript 7 Compiler Migration**:
  - **Native Compiler Tooling**: Upgraded `npm run build` (`tsc -b`) and `npm run typecheck` (`tsc --noEmit`) to utilize the native `@typescript/native` (TypeScript 7.0.2) compiler binary for faster build and typechecking performance.
  - **Dual-Install Setup**: Configured dual-installation with `npm:@typescript/typescript6@~6.0.2` for `typescript-eslint` compatibility alongside `~7.0.2` native compiler binaries.
- **Scoped Local Variables & Typed Function Parameters**:
  - **Event & Function Scoped Local Variables**: Introduced scoped **Local Variables** for Events (`event.locals`) and Custom Functions (`function.locals`) supporting typed values (`number`, `string`, `boolean`) and default value initializers.
  - **Inspector Local Variable Editor**: Built a dedicated **Local Variables** editor section in the Event and Function Inspector details panels to create, rename, retype, edit default values, and delete scoped variables.
  - **Local Variable Actions**: Added new visual action blocks (`set local %1 to %2`, `change local %1 by %2`, and `update local %1 by %2 %3`) supporting direct value assignments and math operations (`add`, `subtract`, `multiply`, `divide`, `remainder`).
  - **Typed Function Argument Sockets**: Custom function call blocks (`actionCallFunction`) automatically generate and render typed input sockets (`ARG_paramId`) for every parameter defined on the target function, automatically syncing with shadow default blocks.
  - **Unified Value Picker Popovers**: Enhanced `ValuePickerField` popovers to search and list scoped **Local Variables** and **Function Parameters** alongside global variables.
  - **Automatic Scope Resolution**: Implemented compiler helper `readScopedLocalId` to automatically resolve local variable block references (`setLocal`, `changeLocal`, `updateLocal`) to the first available local variable in the containing event or function scope.
  - **Scoped Reference Reader**: Updated `readScopedPickerReference` in the Blockly compiler to decode local variable (`local:localId`), function parameter (`parameter:parameterId`), and global variable references smoothly across all value input blocks.
  - **Event & Workspace Variable Badges & Subtle Hover Highlight**: Rendered compact local variable chips (`.event-card-local-chip`) directly on Event Card headers and enhanced both Global Variable rows (`.global-row`) and Workspace Local Variable owner cards (`.workspace-local-owner`). Hovering over Global Variable rows or Event Cards applies a soft, subtle tint, count badge highlight, and border glow to variable items and action pills (`is-local-action`, `is-global-action`).
  - **Configurable Accent & Highlight Color System**: Expanded **Settings ➔ General ➔ Accent & Highlight Color** (`highlightTint`) across the entire engine UI. Introduced **Unified Engine Button Design System** base utility classes (`.btn-primary`, `.btn-icon`, `.btn-pill`, `.btn-tab`) and unified drag-and-drop targets (`.is-drag-over`), folder tree selections (`.workspace-folder-row.is-selected`), context menu items (`.context-menu-item:hover`), and range sliders (`input[type="range"]::-webkit-slider-thumb`) so that all interactive components automatically inherit the active Accent Theme seamlessly. Color presets (**Blue (Default)**, **Emerald Green**, **Royal Purple**, **Sunset Orange**, **Amber Yellow**) dynamically transform Editor Panel tabs (`[Logic Blocks, Scene, Sprites]`), Inspector tabs (`[Actors, Props, UI, Blocks]`), Inspector Blocks sub-mode tabs (`[Events, Functions]`, `[Simple, Advanced]`), Workspace tabs (`[Assets, Scenes, Functions, Groups, Variables, Project]`), Workspace Asset sub-tabs (`[Images, Audio, Sounds, Sound Synth]`), Inspector & Event Sheet Condition and Action cards/pills (`.condition-pill`, `.action-pill`, `.condition-group`), Main Color Input Box/Button (`.sprite-color-field input:hover`, `:focus`), Inspector Header Context Options (`[...]`) button (`.inspector-actor-context-trigger:hover`), Inspector & Sprite Editor Color Swatches & Hover Rings (`.color-preset-swatch:hover`, `.sprite-color-section`, `.sprite-color-swatch:hover`, `.sprite-color-swatch.is-selected`), Sprite Card Options (`[...]`) & Disclosure buttons (`.sprite-asset-context-btn`, `.sprite-asset-disclosure-btn`), Scene Editor Pen & Options buttons (`.scene-actor-sprite-edit`, `.scene-actor-context-trigger`), Subtle Selected Cards & Actor Panel Items (`.scene-actor-list-item.is-selected`, `.sprite-asset-item.is-selected`), Workspace Groups list rows (`enemies`, `hazards`, etc.), Workspace Functions rows/cards, Scene Editor panel dock items (`.scene-actor-list-item`), Scene Editor view mode toggles (`[Scene + panel]`, `[Scene]`, `[Panel]`), Logic Blocks search magnifier toggle button & search text field focus (`.blockly-workspace-search-toggle`, `.blockly-workspace-search`), Topbar right side buttons (dark mode sun/moon toggle, gear settings button), Preview section `[solo]` and `[collapse]` buttons, Event Card `[ping]` and `[delete event]` action buttons, Sound & Audio Play buttons (`[Play]`), asset options buttons (`[...]`), audio player waveforms, Workspace Scene Cards (`.project-scene-card`), action buttons (`[Add Function]`, `[New Prefab]`, `[Add]`, `[+Add Event]`, `[Assign]`), all custom checkboxes (`Visible`, `Colliders`, Settings, Inspector toggles), Inspector Lock toggle, Pane Solo & Hide toggles, collapse/expand buttons, regular button hovers (`Rename`, `+ Local`), Sprite Editor tools, topbar view toggles, card selections, and hover highlights seamlessly.
  - **Scene Editor Actor List Flex Layout Refinements**: Refactored `.scene-actor-list-item`, `.scene-actor-select`, and `.scene-actor-actions` in `App.css` to use a clean flexbox layout (`display: flex`, `gap: 6px`, `margin-left: auto`), ensuring consistent horizontal alignment across actor icons, titles, pen edit controls, and context triggers.
  - **Runtime & Exporter Integration**: Evaluates scoped local variables and function parameters cleanly in the `EventInterpreter` and exported standalone HTML5 runtime script.

### v0.6.5 - Segmented Scene View Mode, Scene Colliders Preferences, and Reorderable Main Panes

- **Icon-Only Segmented Scene View Mode Control**:
  - Replaced the single cycling text button (`[Scene+Panel]`, `[Scene]`, `[Panel]`) in the Scene Editor header toolbar with a compact 3-segment pill control (`.scene-view-segmented`).
  - Icons-only layout (`[ ◧ | 🖼️ | ☰ ]`) for Split View, Scene Canvas Only, and Actor Panel Only options.
  - Sized at a compact **24px** height with strict **1:1 square ratio** (20px × 20px) segment buttons.
  - Styled with a subtle neutral monochrome palette (high-contrast solid white/dark-slate active state, muted unselected icons, zero loud color fills).
- **Scene Toolbar Colliders Preference & Right-Click Context Menu**:
  - Added a new preference in **Settings ➔ Scene** (`Show Colliders toggle in Scene toolbar`, default `true`).
  - Integrated a right-click context menu on the Colliders toggle (`Hide`) across **Editor-Scene** (inside the main Editor pane), the **Scene Preview** viewport, and the **Prefab Editor**, allowing users to right-click and instantly toggle the setting on/off to declutter the toolbar.
- **Reorderable Main Panes**:
  - Added persistent **Move Left** and **Move Right** context actions for Editor, Preview, and Inspector panes. Pane widths, solo mode, responsive layouts, resize separators, and toolbar controls automatically reflect the customized layout order.

### v0.6.4 - 2026-07-22

- **Hierarchical Entity Prefab System**: Implemented a comprehensive prefabs engine (`src/app/prefabs.ts`, `src/engine/PrefabResolver.ts`) allowing users to define template groups of parented entities and nested child prefabs with origin-relative offsets.
  - **Dedicated Prefabs Tab**: Added a **Prefabs** pane in the workspace panel to manage, edit, and instantiate prefabs onto the active scene.
  - **Parent-Child Hierarchical Transforms**: Propagates positions, scales, and rotations recursively down parented entity hierarchies using relative local offsets.
  - **Independent Physics Toggles**: Added an `independentPhysics` property to allow child entities to behave as separate physical body simulations.
  - **Visual Prefab Instances & Overrides**: Rendered prefab instances inside the stage scene editor with purple dashed rectangles and bracketed title tags (e.g. `[MyPrefab]`). Translating or transforming instance bounds automatically propagates offsets to all member child entities. Added support for live, manual, and locked update modes, structural overrides, additions, and removals.
  - **Auto Schema Migrations**: Integrated a Zod preprocessor `migrateLegacyPrefabInstances` inside the project schema to cleanly convert old instances into the new unified structure.
- **Blockly Workspace Block Search**:
  - **Workspace Search Toolbar**: Integrated a search bar inside the Blockly logic sheet workspace, toggleable via the keyboard shortcut `Cmd/Ctrl+F` or the toolbar search button.
  - **Dynamically Indexed Queries**: Searches block text, category names, tooltips, identifiers, and custom values.
  - **Interactive Focus & Match Cycles**: Highlights, centers the workspace camera, and triggers a pulse animation on matches. Cycles matches via arrow buttons or `Enter` / `Shift+Enter` keys, displaying real-time indices.
- **Accessibility, SEO & Website Enhancements**:
  - **Accessibility & Semantic SEO**: Audited the `website` landing page to use semantic tags (e.g. `<main>` layout container), proper HTML input label relationships (`htmlFor` matching), and ARIA attributes (`aria-label`, `aria-hidden`, `aria-expanded`).
  - **Performance-Optimized Website Architecture**: Refactored the massive monolithic landing page into a modular component-based structure (Hero, Features, Arcade, Synth, Header, Footer) with separate stylesheets (`tokens.css`, `layout.css`, `motion.css`, `components.css`).
  - **Lazy Loading & Viewport Observers**: Implemented React `lazy()` code-splitting, a custom `DeferredSection` component to defer rendering resource-heavy modules (like the canvas arcade and synthesizer) until they enter the viewport, and a viewport intersection observer hook (`useLandingPageObservers`) to update navigation states dynamically.
  - **Social Preview Metadata**: Added OpenGraph and Twitter card social preview headers in `index.html`.
  - **Vitest Workspace Integration**: Added `vitest` unit-testing tooling configurations to the website package dependencies.
- **Visual Input Sockets & Value Pickers**:
  - **Slotted Variable & Expression Inputs**: Replaced static number, text, and boolean input fields in actions and conditions (such as timer intervals, cooldown durations, random chances, push force vectors, camera coordinates, variable updates, loop sizes, and log message parameters) with dynamic input value sockets. This enables nesting variables, calculations, and mathematical operators directly into actions.
  - **Value Picker Popover Dropdowns**: Created a custom `ValuePickerField` block field that displays an inline value. Clicking it opens a popover where users can enter a raw literal value (for text/numbers) or select from a searchable list of compatible global variables directly, eliminating the need to drag separate variable blocks.
- **Scoped Local Variables & Typed Function Parameters**:
  - **Event & Function Scoped Local Variables**: Introduced scoped **Local Variables** for Events (`event.locals`) and Custom Functions (`function.locals`) supporting typed values (`number`, `string`, `boolean`) and default value initializers.
  - **Inspector Local Variable Editor**: Built a dedicated **Local Variables** editor section in the Event and Function Inspector details panels to create, rename, retype, edit default values, and delete scoped variables.
  - **Local Variable Actions**: Added new visual action blocks (`set local %1 to %2`, `change local %1 by %2`, and `update local %1 by %2 %3`) supporting direct value assignments and math operations (`add`, `subtract`, `multiply`, `divide`, `remainder`).
...
### v0.6.1 - 14 New Actor Behaviors, Physics Throwing & Drags, and Dynamic Physics Sprite Rotations

- **14 New Specialized Behaviors**: Expanded the behaviors system with 14 new configurable behaviors:
  - **Face Movement (`faceMovement`)**: Automatically rotates/orients the actor to face the direction of its current movement vector.
  - **Scene Edge (`sceneEdge`)**: Constrains actors at screen boundaries (`clamp`, `bounce` off borders, `wrap` around edges, or `stop`).
  - **Bob (`bob`)**: Oscillates position horizontally or vertically in a floating wave animation.
  - **Orbit (`orbit`)**: Rotates an entity in a circular orbit around another target actor.
  - **Keep Distance (`keepDistance`)**: Moves towards or away from a target actor to maintain a specific distance range.
  - **Wander (`wander`)**: Smoothly crawls randomly around the screen.
  - **Move Toward Point (`moveTowardPoint`)**: Moves an entity towards a designated `(x, y)` coordinate.
  - **Pulse (`pulse`)**: Animates the scale size of the actor, oscillating between minimum and maximum scale parameters.
  - **Platformer Controller (`platformerController`)**: Keyboard/gamepad controls tailored for dynamic physics platformers, featuring customizable move speed, acceleration, air control multipliers, jump speed, and coyote time.
  - **Top-Down Controller (`topDownController`)**: Binds movement to keyboard/joysticks with options for diagonal locks, joystick analog tilt sensitivity, and face-movement.
  - **Homing (`homing`)**: Moves towards a target actor with a customizable turn speed limit.
  - **Turret Aim (`turretAim`)**: Rotates/steers the entity to constantly look at a target actor.
  - **Camera Follow (`cameraFollow`)**: Smoothly targets and centers the viewport camera onto this actor.
  - **Lifetime (`lifetime`)**: Automatically destroys the actor entity after a set duration (in seconds).
- **Physics Drag & Throw on Release**: Upgraded **Drag & Drop (`drag`)** to support physics dragging. Dynamic bodies temporarily convert to kinematic when held. It computes throw velocity based on pointer movements and applies a physical velocity throw on release (`throwOnRelease`).
- **Dynamic Physics Rotations**: Integrated angular velocity and rotation updates inside `PhysicsWorld.ts`. Sprites now roll and spin dynamically when colliders are set to circles (e.g. balls, coins) or when `lockRotation` is unchecked, syncing their physics rotation back to the visual canvas rendering.
- **Score Actions Cleanup**: Removed the deprecated `actionSetScore` and `actionChangeScore` compiled actions, unifying scores fully under standard variable structures.
- **Editor Initialization & UI Polish**: Initialized new actors with empty behaviors arrays (`behaviours: []`) and functions with `enabled: true`. Wired the default pixel sprite size dropdown setting into the Blockly Editor pane.
- **Code Cleanups & Type Safety**: Deleted unused `actorShadow` and connection test helpers, and resolved type constraints inside the gesture options dropdown.

### v0.6.0 - Custom Actor Behaviors System, Extended Physics Engine (Planck.js Bindings), Pointer Gestures, Virtual Joystick Settings, and Inspector Thumbnails

- **Custom Actor Behaviors System**: Created a modular behavior framework (`src/engine/behaviours/BehaviourSystem.ts`) to attach reusable movement/interaction scripts to actors. Built a dedicated Behaviors inspector panel to add, reorder, and configure parameters for 6 built-in behaviors:
  - **Drag & Drop (`drag`)**: Drag entities on stage using mouse or touch inputs, supporting axes locking and scene boundary clamping.
  - **Directional Control (`directionalControl`)**: Binds movement to keyboard keys or gamepad sticks, supporting diagonal movement and analog speed scaling.
  - **Spin (`spin`)**: Continuously rotates an actor at a configurable degrees-per-second speed.
  - **Auto Move (`autoMove`)**: Moves an actor automatically in any of 8 directions at a set speed.
  - **Patrol (`patrol`)**: Moves an actor back and forth along a horizontal or vertical path.
  - **Follow Actor (`follow`)**: Steers an actor to chase another target entity, stopping once it reaches a configurable threshold.
- **Extended Physics Engine (Planck.js Bindings)**: Added support for kinematic bodies (move via velocity, unaffected by gravity), horizontal/vertical axis locks (`lockPositionX` / `lockPositionY`), linear damping (air resistance), custom density, one-way platforms (enabling entities to jump through platforms from below but land on top), and layer-based collision filtering (`collisionLayer` / `collidesWith` bitwise masks).
- **Pointer Gestures & Multi-Touch Triggers**: Implemented a pointer gesture parser (tap, double tap, long press, and drag) with a clock-based multi-touch tracking system. Added corresponding Blockly gesture event blocks.
- **Input, Joysticks & Gamepad Diagnostics**: Added configuration settings for virtual joysticks (adjustable deadzone, response curves, and fixed speed options) and physical gamepad enhancements (physical sticks follow preview constraints, digital hat/axis decoding, and idle "ghost" controller rejection).
- **Visual Editor Thumbnails**: Added live-updating sprite and asset thumbnails inside the inspector header, fallback visual icons, and styled block type labels in search lists.
- **Custom Blockly Color Field**: Replaced raw color input text fields with an interactive `BlocklyColorField` that displays a color swatch, transparency indicators, and opens a native popover picker.
- **Function Toggle Options**: Added an `enabled` toggle field to custom functions to temporarily exclude them from compilation.
- **New Playgrounds**: Added three new interactive sample projects (`pointerGestures`, `physicsPlayground`, `behaviourPlayground`) to demonstrate the new capabilities.
...
- **Center-Pivot Coordinate Transition**: Shifted the 2D coordinate system from top-left aligned positions to center-pivot positions `(x, y)` for all entities, simplifying movement and scaling logic.
- **Geometry Helpers Module**: Created `src/engine/Geometry.ts` to manage bounds retrieval (`getEntityBounds`, `getBoxColliderBounds`), circle calculations (`getCircleCollider`), point containment checks (`containsPoint`), and direct center-to-center distance computations (`getEntityCenterDistance`).
- **Pivoted Rendering & Stage Overlays**: Updated the Canvas Renderer (`Renderer.ts`), standalone player builds (`standalone.ts`), and visual stage editor (`SceneEditor.tsx`) to render from the center. Set Konva entity shape offsets to `(width / 2, height / 2)` to align selection boxes and drag nodes.
- **Mutations & Sample Spawning**: Configured project mutations (`projectMutations.ts`) to spawn new elements exactly at scene center coordinates. Adjusted the coordinates of all existing actors, props, and UI boxes in `collectCoins.ts` and `dodgeEnemies.ts` so they remain in their correct visual positions.
- **Modular Collision Provider**: Extracted collision logic into a new `RuntimeCollisionProvider` interface and a `ManualCollisionProvider` helper (`CollisionProvider.ts`). The event interpreter now queries collision results and overlapping pairs through the provider rather than calling entity loops directly.
- **Props Schema Removal**: Deleted `PropSchema` and the `scene.props` field from the project schema (`src/schemas/project.ts`), fully unifying decorative elements under the main `entities` array. Pre-processing rules continue to automatically migrate legacy project formats into unified `kind: 'prop'` entities.
- **Comparison Operator Normalization**: Added a pre-processor rule to `comparisonOperatorSchema` to automatically map double-equals (`==`) to single-equals (`=`) in project JSON schemas, preventing hand-edited files from bypassing event conditions interpreter switch blocks.
- **Touch-Optimized Frame Dragging**: Added pointer event handlers (`onPointerDown`, `onPointerMove`, `onPointerUp`) to manage frame card dragging in the timeline sidebar, allowing mobile and touch-screen users to reorder frames smoothly. Integrated a vertical drag handle icon (`GripVerticalIcon`) to clarify the interaction.
- **Image Preloading Cache**: Added `preloadImageAssets` to the Canvas Renderer to load and cache data URLs upon starting a game, resolving the brief gray placeholder flash that occurred while images were loading asynchronously.
- **Scaled Workspace Trashcan**: Polished the Blockly editor by scaling the workspace trashcan down to `78%` of its original size. Overrode `trashcan.position` to apply a smooth scale-and-translate transform anchored to its resting corner.
...
- `v0.6.7` - Prepared codebase and version manifests for `v0.6.7` feature developments
- `v0.6.5` - Segmented Scene View Mode Control ([Split | Scene | Panel]), Colliders Preference & Right-Click Context Menu, Reorderable Main Panes & Workspace Header Docking (Move Left / Move Right / Header Drag-and-Drop)
- `v0.6.4` - Configurable Accent & Highlight Color System (Blue, Emerald, Purple, Orange, Yellow), Unified Engine Button Design System (.btn-primary, .btn-icon, .btn-pill, .btn-tab), TypeScript 7 Compiler Migration (@typescript/native v7.0.2), Scoped Local Variables (events & functions), typed function argument sockets, ValuePickerField inline popover dropdowns, automatic local variable scope resolution, and standalone runtime exporter updates

Recent checkpoints:

- `v0.6.2` - Hierarchical Entity Prefab System, logic workspace block search toolbar, dynamic variable-slotted input value sockets, custom ValuePickerField popover dropdowns, and performance-optimized marketing website (code-splitting, modular stylesheets, and lazy deferred rendering)
- `v0.6.1` - 14 New Actor Behaviors (faceMovement, sceneEdge, bob, orbit, keepDistance, wander, moveTowardPoint, pulse, platformerController, topDownController, homing, turretAim, cameraFollow, lifetime), physical drag & throw-on-release, dynamic physics-driven sprite rotations (rolling/spinning circle colliders)
- `v0.6.0` - Custom Actor Behaviors System (drag, directionalControl, spin, autoMove, patrol, follow), extended physics (kinematic bodies, axis locks, linear damping, one-way platforms, collision layers/masks), pointer gestures (tap, double tap, long press, drag), virtual joystick configuration, physical gamepad mapping improvements, visual inspector thumbnails, custom Blockly color fields, and toggleable custom functions

- `v0.5.2` - Sprite Editor Animation Mockup preview player tab, onion skinning visual overlays, pixel marquee selection tool, context clipboard/rotation canvas menu actions, interactive duration stretching, compact sidebar settings play controls, stage entity reset actions, side-by-side context button layouts, footer console log checkbox styling refinements, dark mode color scheme integration, browser Gamepad API and low-level WebHID controller fallback support (Switch, PS4, Xbox), custom antialiased native checkbox styling, Collect Coins character dashing, center-pivot coordinate spaces with a geometry utility helper, modular collision provider interfaces, complete removal of deprecated props schemas, comparison operator normalization, touch-screen timeline pointer dragging, preloaded image caches, scaled workspace trashcans, O(1) indexed render loop searches, centralized COLORS, barrel engine exports, audio resource garbage disposal, and test suite expansion (191 tests)
...
- ~~Add Planck physics behind a `PhysicsWorld` adapter~~ (done: optional per-actor `usePhysics`, sensors, grounded, force/impulse/gravity blocks)
